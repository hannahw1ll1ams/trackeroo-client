...
export default class AlternativeMap extends Component {
  ...
  _getLocationAsync = async () => {
    ...
    this.location = await Location.watchPositionAsync({ enableHighAccuracy: true, timeInterval: 1000, distanceInterval: 0.1 }, location => {
    ...
    return (
      <View style={styles.container}>
        ...
        >
          ...
          {this.state.isMapTrue && routeCoordinates.length > 0 && isRunning === false &&
            <Marker coordinate={{ latitude: routeCoordinates.slice(-1)[0].latitude, longitude: routeCoordinates.slice(-1)[0].longitude }}
              title="Finish"
            />
          }
