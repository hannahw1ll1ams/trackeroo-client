    ...
    return (
      <View style={styles.wholeContainer}>
        ...
        {peopleRunning.length === 0 && (
          <Text style={styles.text}>Start a run?</Text>
        )}
        {peopleRunning.length === 0 && (
          <Button
            title={!this.state.joiningRun ? 'START' : 'HIDE'}
            onPress={this.toggleStopWatch}
          />
        )}

        {peopleRunning.length > 0 && (
          <Text style={styles.text}>Currently Running:</Text>
        )}
        {peopleRunning.length > 0 &&
        ...
        {peopleRunning.length > 0 && (
          <Text style={styles.statsBox}>JOHN's LIVE DISTANCE, TIME, SPEED</Text>
        )}

        {peopleRunning.length > 0 && (
          <Button
            title={!this.state.joiningRun ? 'JOIN' : 'HIDE'}
            onPress={this.toggleStopWatch}
          />
        )}

        {this.state.joiningRun === true && (
          <AlternativeStopWatch
            style={styles.stopwatch}
            updateActivityStatus={this.updateActivityStatus}
          />
        )}
      </View>
    );
